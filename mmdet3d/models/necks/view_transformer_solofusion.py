...
class ViewTransformerSOLOFusion(ViewTransformerLiftSplatShoot):
    def __init__(self, 
        ...
        if self.do_history_stereo_fusion:
            ...
            self.stereo_sampling_num = stereo_sampling_num  # 7
    ...
    def forward(self, 
                curr_sem_feats,     # curr_img_encoder_feats
                rots, trans, intrins, post_rots, post_trans,
                curr_stereo_feats=None, prev_stereo_feats=None, 
                prev_global2img=None, prev_img_forward_aug=None, curr_global2img=None, curr_img_forward_aug=None, curr_unaug_cam_to_prev_unaug_cam=None, do_stereo_fusion=True):

        B, N, sem_C, sem_H, sem_W = curr_sem_feats.shape

        curr_sem_feats = curr_sem_feats.view(B * N, sem_C, sem_H, sem_W)
        curr_img_feat = self.featnet(curr_sem_feats)    # 单层卷积，[B * N, self.numC_input, H, W] -> [B * N, self.numC_Trans, H, W]
        mono_depth_digit = self.get_mono_depth(curr_sem_feats.view(B, N, sem_C, sem_H, sem_W), rots, trans, intrins, post_rots, post_trans) # [B * N, D, H, W]

        # if not self.do_history_stereo_fusion:
        if not do_stereo_fusion:
            ...
            with torch.no_grad():
                ...
                gauss_sample_distr_log = mono_depth_digit.log_softmax(dim=1)    # [B * N, D, H, W]
                gauss_sample_depth_idxs = []
                for _ in range(self.stereo_sampling_num):   # 7
