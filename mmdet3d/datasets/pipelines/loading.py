...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def img_transform(self, img, post_rot, post_tran,
                      ...
                      flip, rotate):
        # adjust image
        img = self.img_transform_core(img, resize_dims, crop, flip, rotate) # 变换后的图像

        # post-homography transformation
        # post_rot = np.eye(2), post_tran = np.zeros(2)
        post_rot *= resize
    ...
    def choose_cams(self):
        if self.is_train and self.data_config['Ncams'] < len(self.data_config['cams']):
            cams = np.random.choice(self.data_config['cams'], self.data_config['Ncams'],    # 从cams中选择Ncams个元素，不可重复选择
                                    replace=False)
        else:
    ...
    def sample_augmentation(self, H , W, flip=None, scale=None):
        ...
        if self.is_train:
            ...
            resize += np.random.uniform(*self.data_config['resize'])    # self.data_config['resize']是一个区间，例如(-0.06, 0.11)
            resize_dims = (int(W * resize), int(H * resize))
            newW, newH = resize_dims
            crop_h = int((1 - np.random.uniform(*self.data_config['crop_h'])) * newH) - fH  # self.data_config['crop_h']: (0.0, 0.0)
            crop_w = int(np.random.uniform(0, max(0, newW - fW)))
            crop = (crop_w, crop_h, crop_w + fW, crop_h + fH)
            flip = self.data_config['flip'] and np.random.choice([0, 1])
            rotate = np.random.uniform(*self.data_config['rot'])    # (-5.4, 5.4)
        else:
            resize = float(fW)/float(W)
            resize += self.data_config.get('resize_test', 0.0)  # 默认值是0.0
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            # resize是比例，resize_dims是resize后大小，crop是图中的范围，flip是是否反转，rotate是旋转角度
            # 随机采样一个变换
            ...
            imgs.append(self.normalize_img(img))    # 加入变换后的图像
        ...
        '''
        到此为止，imgs:[transformed_img (cam 0), transformed_adjacent_img_0 (cam 0), transformed_img (cam 1), transformed_adjacent_img_0 (cam 1), ..., ]
        intrins: [intrinsic_cam_0, intrinsic_cam_1, ..., intrinsic_cam_N-1]
        rots: [cam_0到lidar坐标系的旋转变换, ..., cam_N-1到lidar坐标系的旋转变换]
        trans: [cam_0到lidar坐标系的平移变换, ..., cam_N-1到lidar坐标系的平移变换]
        post_rots: [transformed_img_0变换同时计算的rotation矩阵, ..., transformed_img_N-1变换同时计算的rotation矩阵]
        post_tran: [transformed_img_0变换同时计算的translation矩阵, ..., transformed_img_N-1变换同时计算的translation矩阵]
        '''
        
        # print(len(imgs), len(rots), len(trans), len(intrins), len(post_rots), len(post_trans))
        # [6, 3, 256, 704]  [6, 3, 3]   [6, 3]       [6, 3, 3]      [6, 3, 3]        [6, 3]            for bevdet
        # [12, 3, 256, 704] [6, 3, 3]   [6, 3]       [6, 3, 3]      [6, 3, 3]        [6, 3]            for bevdet4d
        ...
        if self.sequential:
            ...
            # 相邻帧只计算translation
                if not type(results['adjacent']) is list:
                    ...
                    # 对于相机到lidar变换，记录相邻帧相机坐标系到当前帧lidar坐标系的translation变换
                    rots.extend(rots)
                    if self.aligned:
                        posi_curr = np.array(results['curr']['ego2global_translation'], dtype=np.float32)   # The translation from the ego vehicle to global coordinates. (1x3 list)
                        posi_adj = np.array(results['adjacent']['ego2global_translation'], dtype=np.float32)
                        shift_global = posi_adj - posi_curr

                        l2e_r = results['curr']['lidar2ego_rotation']   # The rotation from lidar to ego vehicle. (1x4 list in the quaternion format)
                        e2g_r = results['curr']['ego2global_rotation']
                        l2e_r_mat = Quaternion(l2e_r).rotation_matrix
                        e2g_r_mat = Quaternion(e2g_r).rotation_matrix

                        # shift_global = np.array([*shift_global[:2], 0.0])
                        shift_lidar = shift_global @ np.linalg.inv(e2g_r_mat).T @ np.linalg.inv(l2e_r_mat).T    # shift_global * global_to_ego * ego_to_lidar
                ...
                if not type(results['adjacent']) is list:
                    ...
                    if self.aligned:
                        ...
                        lidaradj2lidarcurr = np.linalg.inv(lidar2ego) @ np.linalg.inv(egocurr2global) \
                                             @ egoadj2global @ lidar2ego    # lidaradj -> egoadj -> global -> egocurr -> lidarcurr
                        trans_new = []
                        rots_new = []
                        for tran,rot in zip(trans, rots):
                            ...
                            mat[:3,:3] = rot
                            mat[:3,3] = tran
                            mat = lidaradj2lidarcurr @ mat  # cam2lidar * lidaradj2lidarcurr -> cam2lidarcurr
        ...
        到此为止，imgs:[transformed_img (cam 0), transformed_adjacent_img_0 (cam 0), transformed_img (cam 1), transformed_adjacent_img_0 (cam 1), ..., ]
        intrins: [intrinsic_cam_0, intrinsic_cam_1, ..., intrinsic_cam_N-1]
        rots: [cam_0到lidar坐标系的旋转变换, ..., cam_N-1到lidar坐标系的旋转变换, adjecent_cam_0到lidar坐标系的旋转变换, ..., adjecent_cam_N-1到lidar坐标系的旋转变换]
        trans: [cam_0到lidar坐标系的平移变换, ..., cam_N-1到lidar坐标系的平移变换, adjecent_cam_0到lidar坐标系的平移变换, ..., adjecent_cam_N-1到lidar坐标系的平移变换]
        post_rots: [transformed_img_0变换同时计算的rotation矩阵, ..., transformed_img_N-1变换同时计算的rotation矩阵, adjacent_transformed_img_0变换同时计算的rotation矩阵, ..., adjacent_transformed_img_N-1变换同时计算的rotation矩阵]
        post_tran: [transformed_img_0变换同时计算的translation矩阵, ..., transformed_img_N-1变换同时计算的translation矩阵, adjacent_transformed_img_0变换同时计算的translation矩阵, ..., adjacent_transformed_img_N-1变换同时计算的translation矩阵]
        '''
        ...
        # print(imgs.shape, rots.shape, trans.shape, intrins.shape, post_rots.shape, post_trans.shape)
        # [6, 3, 256, 704]  [6, 3, 3]   [6, 3]       [6, 3, 3]      [6, 3, 3]        [6, 3]            for bevdet
        # [12, 3, 256, 704] [12, 3, 3]  [12, 3]      [12, 3, 3]     [12, 3, 3]       [12, 3]           for bevdet4d
