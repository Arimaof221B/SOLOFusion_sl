...
class NuScenesDataset(Custom3DDataset):
    ...
    def _set_sequence_group_flag(self):
        ...
        """
        # 划分group，非第一帧且 # of sweeps是0，则是新的sequence
        res = []
        curr_sequence = 0
        ...
        if self.sequences_split_num != 1:
            if self.sequences_split_num == 'all':
                ...
                bin_counts = np.bincount(self.flag) # np.bincount([0, 0, 1, 2, 2]) -> [2, 1, 2]，即获取每个group的长度
                new_flags = []
                curr_new_flag = 0
                for curr_flag in range(len(bin_counts)):
                    curr_sequence_length = np.array(
                        list(range(0, 
                                ...
                                math.ceil(bin_counts[curr_flag] / self.sequences_split_num)))   # 每个group划分self.sequences_split_num个sequence
                        + [bin_counts[curr_flag]])  # 以bin_counts[curr_flag]=18，self.sequences_split_num=4为例，[0, 4, 8, 12, 16, 18]

                    for sub_seq_idx in (curr_sequence_length[1:] - curr_sequence_length[:-1]):  # 错位相减
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            elif self.img_info_prototype == 'solofusion_sequential':
                if info['next'] is None:
                    adjacent = 'next'
                    select_id = -1
                    input_dict.update(dict(img_info=info['cams'],
                                       curr=info,
                                       adjacent=dict(timestamp=info['timestamp'], cams=info['cams'], ego2global_translation=info['ego2global_translation'], ego2global_rotation=info['ego2global_rotation']),
                                       adjacent_type=adjacent))
                else:
                    adjacent = 'next'
                    if type(info[adjacent]) is list:
            ...
            if self.use_sequence_group_flag:
                ...
                input_dict['start_of_sequence'] = index == 0 or self.flag[index - 1] != self.flag[index]    # 布尔值
                ...
                if self.img_info_prototype == 'solofusion_sequential':
                    input_dict['curr_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                        self.data_infos[index], self.data_infos[index + select_id + 1],
                        "lidar", "lidar"))
                    
                    if select_id == 15:
                        input_dict['future_to_future_prev_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                            self.data_infos[index + select_id + 1], self.data_infos[index + select_id],
                            "lidar", "lidar"))
                    else:
                        input_dict['future_to_future_prev_lidar_rt'] = torch.eye(4).to(input_dict['curr_to_future_lidar_rt'])
                    input_dict['global_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                        self.data_infos[index + select_id + 1], self.data_infos[index + select_id + 1],
                        "global", "lidar"))
                    
                    input_dict['future_id'] = select_id + 1
                
                                
                # if not input_dict['future_out_of_sequence']:
                #     input_dict['curr_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                #         self.data_infos[index], self.data_infos[index + 16],
                #         "lidar", "lidar"))
                #     input_dict['global_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                #     self.data_infos[index + 16], self.data_infos[index + 16],
                #     "global", "lidar"))
                # else:
                #     for i in range(1, 17):
                #         if index + i >= len(self.data_infos) or self.flag[index + i] != self.flag[index]:
                #             break
                #     input_dict['curr_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                #         self.data_infos[index], self.data_infos[index + i - 1],
                #         "lidar", "lidar"))
                #     input_dict['global_to_future_lidar_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                #     self.data_infos[index + i - 1], self.data_infos[index + i - 1],
                #     "global", "lidar"))
