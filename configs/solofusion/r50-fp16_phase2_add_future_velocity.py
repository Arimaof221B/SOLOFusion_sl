...
car	0.573	0.465	0.167	0.180	0.370	0.210
truck	0.358	0.618	0.229	0.179	0.240	0.212
bus	0.394	0.712	0.231	0.151	0.588	0.282
trailer	0.158	0.957	0.243	0.580	0.205	0.096
construction_vehicle	0.149	0.837	0.480	1.400	0.117	0.355
pedestrian	0.418	0.677	0.295	0.838	0.432	0.245
motorcycle	0.418	0.554	0.273	1.065	0.423	0.220
bicycle	0.431	0.443	0.257	1.267	0.142	0.008
traffic_cone	0.593	0.410	0.383	nan	nan	nan
barrier	0.567	0.419	0.281	0.189	nan	nan
"""
...
work_dir = None
load_from = None
resume_from = "work_dirs/r50-fp16_phase1/iter_10548.pth"
...
do_future = True    
future_cat_num = 4
future_cat_conv_out_channels = 160
...
model = dict(
    ...
    use_gt_velocity=True, 

    # Short-Term Fusion
    do_history_stereo_fusion=do_history_stereo_fusion,
    history_stereo_prev_step=history_stereo_prev_step,
    
    do_future_stereo_fusion = do_history_stereo_fusion, 
    future_stereo_prev_step=future_stereo_prev_step,
...
data_root = 'data/nuscenes_sp/'
file_client_args = dict(backend='disk')

train_pipeline = [
    dict(type='LoadMultiViewImageFromFiles_BEVDet', is_train=True, sequential=True, aligned=True, trans_only=False, 
         data_config=data_config),
    dict(
    ...
    dict(type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d'], 
         meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'prev_lidar_to_global_rt', 'sample_index', 'gt_velocity', 'gt_boxes', 'timestamp', 'curr_to_future_lidar_rt', 'global_to_future_lidar_rt', 'future_id', 'future_to_future_prev_lidar_rt'))
]

test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles_BEVDet', data_config=data_config, sequential=True, aligned=True, trans_only=False),
    dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_infos_train_4d_interval1_max16.pkl',
        ...
        next_only=True,
        fix_direction=None,
        img_info_prototype='solofusion_sequential',
    ...
    val=dict(pipeline=test_pipeline, 
             classes=class_names,
             data_root=data_root,
             ann_file=data_root + 'nuscenes_infos_val_4d_interval1_max16.pkl',
             modality=input_modality, 
             max_interval=4, 
             min_interval=3, 
             next_only=True, 
