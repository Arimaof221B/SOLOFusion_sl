...
model = dict(
    ...
    before=True,
    img_backbone=dict(
        pretrained='/home/arima/.cache/torch/hub/checkpoints/resnet50-0676ba61.pth',
        type='TSMResNet',
        depth=50,
        output_stride=32, 
        out_indices=(2, 3),
...
train_pipeline = [
    dict(type='LoadMultiViewImageFromFiles_BEVDet', is_train=True, data_config=data_config,
         sequential=True, aligned=True, trans_only=False),  # results['img_inputs'] = self.get_inputs(results)
    dict(
        ...
        load_dim=5,
        use_dim=5,
        file_client_args=file_client_args), # results['points'] = points
    ...
    dict(type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d'], # data[key] = result[key]
         meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',          # img_metas[meta_key] = results[meta_key], data['img_metas'] = DC(img_metas, cpu_only=True)
