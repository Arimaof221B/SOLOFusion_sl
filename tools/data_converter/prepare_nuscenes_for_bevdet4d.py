...
def add_adj_info():
    ...
    for set in ['train', 'val']:
        dataroot = './data/nuscenes_sp'
        dataset = pickle.load(open(os.path.join(dataroot, 'nuscenes_infos_%s.pkl' % set), 'rb'))
            ...
            for adj in ['next', 'prev']:
                ...
                for cam in ['CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_FRONT_LEFT',
                    ...
                    while count < max_adj:
                        ...
                        sample_data = sd_adj
                        if sd_adj['filename'].split('/')[0] != 'samples':
                            continue
                        print(sd_adj)
        ...
        with open(os.path.join(dataroot, 'nuscenes_infos_%s_4d_interval%d_max%d.pkl' % (set, interval, max_adj)), 'wb') as fid:
